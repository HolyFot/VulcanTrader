"""Minimal HyperStrategyMixin port.

Hyperopt is not (yet) supported in VulcanTrader. This module provides just
enough surface for ``IStrategy`` to subclass it without pulling in the full
hyperopt subsystem.

Behaviour summary:
  * Hyperoptable ``Parameter`` attributes declared on a strategy still work —
    their ``.value`` attribute is honoured (the default value supplied at
    construction).
  * ``ft_load_hyper_params``/``ft_load_params_from_file`` are no-ops.
  * Loading of overrides from a sibling ``<Strategy>.json`` file is supported
    via :func:`HyperoptTools.load_params` if it exists; otherwise skipped.
"""
...
from VulcanTrader.hyperopt.hyperopt_tools import HyperoptTools
from VulcanTrader.strategy.parameters import BaseParameter
from VulcanTrader.util.exceptions import OperationalException
from VulcanTrader.util.misc import deep_merge_dicts
...
def detect_all_parameters(
    ...
    """Detect all hyperoptable parameters declared on ``obj``."""
    ...
    for attr_name in dir(obj):
        ...
        if attr.space is None:
            raise OperationalException(
                f"Cannot determine parameter space for {attr_name}."
            )
        attr.name = attr_name
