...
from VulcanTrader.config.deprecated_settings import process_temporary_deprecated_settings
from VulcanTrader.config.directory_operations import create_datadir, create_userdata_dir
from VulcanTrader.config.environment_vars import environment_vars_to_dict
from VulcanTrader.config.load_config import load_file, load_from_files
from VulcanTrader.config.timerange import TimeRange  # noqa: F401  (re-exported)
from VulcanTrader.config.config_secrets import (  # noqa: F401  (re-exported)
...
from VulcanTrader.util.logger import setup_logging
from VulcanTrader.util.misc import deep_merge_dicts, parse_db_uri_for_logging, safe_value_fallback
...
class Configuration:
    ...
    def load_config(self) -> dict[str, Any]:
        ...
        from VulcanTrader.util.arguments import NO_CONF_ALLOWED
    ...
    def _process_data_options(self, config: Config) -> None:
        ...
        config.setdefault("new_pairs_days", 30)
        config.setdefault("dataformat_ohlcv", "feather")
        config.setdefault("dataformat_trades", "feather")
        self._args_to_config(
