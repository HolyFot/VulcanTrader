...
from VulcanTrader.util.config_schema import (
    CONF_SCHEMA,
...
from VulcanTrader.config.deprecated_settings import process_deprecated_setting
from VulcanTrader.constants import UNLIMITED_STAKE_AMOUNT
...
def _extend_validator(validator_class):
    """
    Extended validator for the VulcanTrader configuration JSON Schema.
...
VulcanTraderValidator = _extend_validator(Draft4Validator)
...
def validate_config_schema(conf: dict[str, Any], preliminary: bool = False) -> dict[str, Any]:
    ...
    try:
        VulcanTraderValidator(conf_schema).validate(conf)
        return conf
    except ValidationError as e:
        logger.critical(f"Invalid configuration. Reason: {e}")
        result = best_match(VulcanTraderValidator(conf_schema).iter_errors(conf))
...
def _validate_edge(conf: dict[str, Any]) -> None:
    ...
    if conf.get("edge", {}).get("enabled"):
        raise ConfigurationError(
            "Edge is no longer supported and has been removed from VulcanTrader with 2025.6."
        )
...
def _validate_freqai_include_timeframes(conf: dict[str, Any], preliminary: bool) -> None:
    ...
    if freqai_enabled:
        ...
        from VulcanTrader.exchange import timeframe_to_seconds
