...
Ported from VulcanTrader.configuration.deprecated_settings.
"""
...
def check_conflicting_settings(
    ...
    if name_new in section_new_config and name_old in section_old_config:
        ...
        raise OperationalException(
            ...
            "This deprecated setting will be removed in the next versions of VulcanTrader. "
...
def process_deprecated_setting(
    ...
    if name_old in section_old_config:
        ...
        logger.warning(
            ...
            "will be removed in the next versions of VulcanTrader. "
...
def process_temporary_deprecated_settings(config: Config) -> None:
    ...
    if config.get("telegram"):
        ...
        process_deprecated_setting(
            config["telegram"],
            "notification_settings", "sell_fill",
            "notification_settings", "exit_fill",
        )
        process_deprecated_setting(
            config["telegram"],
            "notification_settings", "sell_cancel",
            "notification_settings", "exit_cancel",
        )
        process_deprecated_setting(
            config["telegram"],
            "notification_settings", "buy",
            "notification_settings", "entry",
        )
        process_deprecated_setting(
            config["telegram"],
            "notification_settings", "buy_fill",
            "notification_settings", "entry_fill",
        )
        process_deprecated_setting(
            config["telegram"],
            "notification_settings", "buy_cancel",
