...
from VulcanTrader.constants import FTHYPT_FILEVERSION, LAST_BT_RESULT_FN, Config
from VulcanTrader.enums import HyperoptState
from VulcanTrader.util.misc import file_dump_json, plural
from VulcanTrader.hyperopt.hyperopt.hyperopt_optimizer import INITIAL_POINTS, HyperOptimizer
from VulcanTrader.hyperopt.hyperopt.hyperopt_output import HyperoptOutput
from VulcanTrader.hyperopt.hyperopt_tools import (
    HyperoptStateContainer,
    HyperoptTools,
    hyperopt_serializer,
)
from VulcanTrader.util import get_progress_tracker
...
class Hyperopt:
    ...
    def start(self) -> None:
        ...
        # Python 3.14 changed ABCMeta internals making _abc._abc_data non-picklable
        # via cloudpickle/loky. Backtesting is also not thread-safe (shared class lists),
        # so the only correct option on Python 3.14 + Windows is single-process.
        if sys.version_info >= (3, 14) and config_jobs != 1:
            logger.warning(
                "Python 3.14 detected: parallel hyperopt is not supported due to "
                "ABCMeta pickling incompatibility. Forcing -j 1 (single process)."
            )
            config_jobs = 1
        try:
