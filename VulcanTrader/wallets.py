...
from VulcanTrader.constants import UNLIMITED_STAKE_AMOUNT, Config, IntOrInf
from VulcanTrader.enums import RunMode, TradingMode
from VulcanTrader.util.exceptions import DependencyException
from VulcanTrader.exchange import Exchange
from VulcanTrader.util.misc import safe_value_fallback
from VulcanTrader.persistence import LocalTrade, Trade
...
class Wallets:
    ...
    def is_max_open_ratio_reached(self) -> bool:
        """
        Return True when total open stake meets or exceeds max_open_ratio * wallet balance.
        Hard-gates new entries in the bot and backtesting engine.
        Configure via 'max_open_ratio' (float 0.0-1.0) in the config file.
        """
        max_open_ratio = self._config.get('max_open_ratio')
        if max_open_ratio is None:
            return False
        return Trade.total_open_trades_stakes() >= self.get_total_stake_amount() * max_open_ratio
