...
from VulcanTrader.constants import Config, LongShort
from VulcanTrader.util.exceptions import ConfigurationError
from VulcanTrader.persistence import PairLocks
from VulcanTrader.persistence.models import PairLock
from VulcanTrader.util.protections import IProtection
from VulcanTrader.resolvers import ProtectionResolver
...
class ProtectionManager:
    def __init__(self, config: Config, protections: list) -> None:
        ...
        if protections:
            # Protections are a stubbed-out subsystem in VulcanTrader; surface
            # this once at DEBUG so it doesn't spam every startup.
            logger.debug(
                "Ignoring %d protection handler(s) from strategy: %s "
                "(protections subsystem is not implemented)",
                len(protections),
                [p.get("method", "?") for p in protections],
            )
            return
