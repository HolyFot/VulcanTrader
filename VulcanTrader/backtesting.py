...
# =============================================================================
#  VulcanTrader  ::  Backtesting engine
# =============================================================================
#
#  Purpose
#  -------
#  Replays historical OHLCV data against an `IStrategy` implementation to
#  produce a complete trade ledger, equity curve and performance statistics
#  identical in shape to the live-trading output. Used directly by the CLI
#  ``backtest`` command and indirectly by the Hyperopt loop (which calls
#  :meth:`Backtesting.backtest` once per parameter set).
#
#  Pipeline
#  --------
#      load_bt_data()             ── fetch + slice OHLCV for the timerange
#         │
#         ▼
#      _get_ohlcv_as_lists()      ── pre-compute signal columns into tuples
#         │
#         ▼
#      time_pair_generator()      ── candle-by-candle iteration across pairs
#         │
#         ▼
#      backtest_loop()            ── per-candle entry/exit/adjust decisions
#         │
#         ▼
#      generate_backtest_stats()  ── aggregate trades into BacktestResultType
#         │
#         ▼
...
#  Provenance
#  ----------
#  Ported from the VulcanTrader reference implementation. Differences vs. ref:
#    * FreqAI support has been removed entirely.
#    * Imports adjusted for the VulcanTrader package layout:
#          ft_types         → util.backtest_result_type
#          mixins           → util.logging_mixin
#          bt_progress      → util.bt_progress
#          plugins.*        → util.pairlistmanager / util.protectionmanager
#          FtPrecise        → util.ft_precise
#          dt_now           → util.datetime_helpers
#          optimize_reports → util.optimize_reports
#
#  Notes
#  -----
#  * Database persistence is **disabled** during backtesting; all trades live
#    in :class:`LocalTrade` in-memory containers (see ``Trade.reset_trades()``
#    and :meth:`disable_database_use` in ``__init__``).
#  * Stop-loss-on-exchange is force-disabled in :meth:`_set_strategy` because
#    backtesting assumes a "perfect" stoploss fill at the configured price.
#  * Indexes ``DATE_IDX``..``EXIT_TAG_IDX`` below are tightly coupled to the
#    ``HEADERS`` list — any reordering MUST update both.
# =============================================================================

"""Backtesting engine for VulcanTrader (see header comment above for details)."""
...
from VulcanTrader.constants import DATETIME_PRINT_FORMAT, Config, IntOrInf, LongShort
from VulcanTrader.data import history
from VulcanTrader.data.btanalysis import (
    find_existing_backtest_stats,
    get_tick_size_over_time,
    trade_list_to_dataframe,
)
from VulcanTrader.data.converter import trim_dataframe, trim_dataframes
from VulcanTrader.data.dataprovider import DataProvider
from VulcanTrader.data.metrics import combined_dataframes_with_rel_mean
...
from VulcanTrader.exchange.exchange import TICK_SIZE, Exchange
from VulcanTrader.exchange.exchange_utils import amount_to_contract_precision, price_to_precision
...
from VulcanTrader.util.backtest_caching import get_strategy_run_id
from VulcanTrader.persistence import (
    CustomDataWrapper,
    LocalTrade,
    Order,
    PairLocks,
...
from VulcanTrader.util.bt_progress import BTProgress
from VulcanTrader.util.datetime_helpers import dt_now
...
from VulcanTrader.util.optimize_reports import (
...
class Backtesting:
    ...
    def load_bt_data(self) -> tuple[dict[str, DataFrame], TimeRange]:
        ...
        self._unavailable_pairs: list[str] = []
        self._load_bt_data_detail()

        # Remove pairs that have no futures data (funding/mark rate missing)
        for pair in self._unavailable_pairs:
            data.pop(pair, None)
        ...
        if self.trading_mode == TradingMode.FUTURES:
            ...
            for pair in self.pairlists.whitelist:
                ...
                if pair not in funding_rates_dict or pair not in mark_rates_dict:
                    unavailable_pairs.append(pair)
                    logger.warning(
                        f"Pair {pair} has no funding/mark rate data - skipping."
                    )
                    continue
            ...
            if unavailable_pairs:
                logger.warning(
                    f"Pairs {', '.join(unavailable_pairs)} have no funding/mark rate data "
                    "and will be excluded from backtesting."
                )
                self._unavailable_pairs = unavailable_pairs
        else:
            self.futures_data = {}
    ...
    def _get_ohlcv_as_lists(self, processed: dict[str, DataFrame]) -> dict[str, tuple]:
        ...
        for pair in processed.keys():
            ...
            df_analyzed = self.strategy.trader_advise_signals(pair_data, {"pair": pair})
        ...
        if exit_.exit_flag:
            ...
            if exit_.exit_type in (
                ...
                if order_type == "limit":
                    ...
                    # VulcanTrader does not support this in live, and the order would fill immediately
                    if trade.is_short:
    ...
    def get_valid_entry_price_and_stake(
        ...
        if order_type == "limit":
            ...
            # which VulcanTrader does not support in live.
    ...
    def check_order_cancel(
        ...
        """
        timedout = self.strategy.trader_check_timed_out(
            trade,  # type: ignore[arg-type]
            order,
            current_time,
        )
    ...
    def start(self) -> None:
        ...
        # Clean up exchange connection to avoid unclosed connector warnings
        self.exchange.close()
