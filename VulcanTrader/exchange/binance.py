...
from VulcanTrader.constants import DEFAULT_DATAFRAME_COLUMNS
from VulcanTrader.enums import TRADE_MODES, CandleType, MarginMode, PriceType, RunMode, TradingMode
from VulcanTrader.util.exceptions import DDosProtection, OperationalException, TemporaryError
from VulcanTrader.exchange import Exchange
from VulcanTrader.exchange.binance_public_data import (
    concat_safe,
    download_archive_ohlcv,
    download_archive_trades,
)
from VulcanTrader.exchange.common import retrier
from VulcanTrader.exchange.exchange_types import TraderHas, Tickers
from VulcanTrader.exchange.exchange_utils_timeframe import timeframe_to_msecs
from VulcanTrader.util.misc import deep_merge_dicts, json_load
...
class Binance(Exchange):
        ...
        try:
            if self.trading_mode == TradingMode.FUTURES and not self._config["dry_run"]:
                ...
                if position_side.get("dualSidePosition") is True:
                    msg += (
                        "\nHedge Mode is not supported by VulcanTrader. "
                ...
                if (
                    ...
                    msg += (
                        "\nMulti-Asset Mode is not supported by VulcanTrader. "
    ...
    def funding_fee_cutoff(self, open_date: datetime):
        ...
        https://github.com/VulcanTrader/VulcanTrader/pull/5779#discussion_r740175931
        ...
        if not self._config["exchange"].get("only_from_ccxt", False):
            if from_id is None or not since:
                trades = await self._api_async.fetch_trades(
                    pair,
                    params={
                        self.trader_has["trades_pagination_arg"]: "0",
                    },
                    limit=5,
                )
    ...
    def _get_spot_pair_delist_time(self, pair: str, refresh: bool = False) -> datetime | None:
        ...
        for schedule in delist_schedule:
            ...
            for symbol in schedule["symbols"]:
                ...
                if trader_symbol is None:
                    continue

                cache[trader_symbol] = delist_dt
