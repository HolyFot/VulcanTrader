...
from VulcanTrader.constants import Config, PairWithTimeframe
from VulcanTrader.enums.candletype import CandleType
from VulcanTrader.util.exceptions import TemporaryError
from VulcanTrader.exchange.common import retrier
from VulcanTrader.exchange.exchange import timeframe_to_seconds
from VulcanTrader.exchange.exchange_types import OHLCVResponse
from VulcanTrader.util import dt_ts, format_ms_time, format_ms_time_det
...
class ExchangeWS:
    ...
    def _handle_exception(self, loop: asyncio.AbstractEventLoop, context: dict) -> None:
        """Custom exception handler for asyncio loop to suppress connection reset errors."""
        exception = context.get("exception")
        message = context.get("message", "")

        if exception and hasattr(exception, "winerror") and exception.winerror == 10054:
            logger.debug(f"[WinError 10054] Connection forcibly closed by remote host (suppressed): {exception}")
            return
        if isinstance(exception, (ConnectionResetError, OSError, ConnectionError)):
            if "10054" in str(exception) or "forcibly closed" in str(exception).lower():
                logger.debug(f"Connection reset in WS loop (suppressed): {exception}")
                return
        if "forcibly closed" in message.lower() or "connection reset" in message.lower() or "10054" in message:
            logger.debug(f"Connection error in WS loop (suppressed): {message}")
            return
    ...
    def get_ohlcv_with_refresh(
        ...
        """
        Get deepcopied klines and the last refresh time
        """
    ...
    async def _unwatch_ohlcv(self, pair: str, timeframe: str, candle_type: CandleType) -> None:
        try:
            ...
            # Network errors are common on shutdown - connection is likely already closed.
        ...
        while True:
            ...
            try:
                ...
                if consecutive_errors >= max_consecutive_errors:
                    logger.error(
                        f"Connection reset for {pair}, {timeframe}: {e}. "
                        f"Max retries ({max_consecutive_errors}) exceeded, giving up."
                    )
                    break
                logger.warning(
                    f"Connection reset for {pair}, {timeframe}: {e}. "
                    f"Reconnecting in {retry_delay:.1f}s (attempt {consecutive_errors})..."
                )
                ...
                if consecutive_errors >= max_consecutive_errors:
                    ...
                    break
                logger.warning(
                    f"CCXT error for {pair}, {timeframe}: {e}. "
                    f"Reconnecting in {retry_delay:.1f}s (attempt {consecutive_errors})..."
                )
                await asyncio.sleep(retry_delay)
                retry_delay = min(retry_delay * 2, max_retry_delay)
                ...
                if consecutive_errors >= max_consecutive_errors:
                    logger.error(
                        f"Unhandled exception in watch loop for {pair}, {timeframe}: {e}. "
                        f"Max retries ({max_consecutive_errors}) exceeded, giving up."
                    )
                    break
                logger.debug(
                    f"Connection error for {pair}, {timeframe}: {e}. "
                    f"Reconnecting in {retry_delay:.1f}s (attempt {consecutive_errors})..."
                )
                await asyncio.sleep(retry_delay)
                retry_delay = min(retry_delay * 2, max_retry_delay)
