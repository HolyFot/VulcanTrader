...
from VulcanTrader.exchange.exchange_utils_timeframe import timeframe_to_minutes, timeframe_to_prev_date
from VulcanTrader.util.valid_exchanges_type import TradeModeType, ValidExchangesType
from VulcanTrader.util import FtPrecise
...
def is_exchange_known_ccxt(exchange_name: str, ccxt_module: CcxtModuleType | None = None) -> bool:
    return exchange_name in ccxt_exchanges(ccxt_module) or exchange_name in NON_CCXT_EXCHANGES


def ccxt_exchanges(ccxt_module: CcxtModuleType | None = None) -> list[str]:
    """
    Return the list of all exchanges known to ccxt (plus custom non-ccxt exchanges)
    """
    exchanges = ccxt_module.exchanges if ccxt_module is not None else ccxt.exchanges
    return list(exchanges) + NON_CCXT_EXCHANGES
...
def validate_exchange(exchange: str) -> tuple[bool, str, ccxt.Exchange | None]:
    ...
    """
    # Handle non-CCXT custom exchanges (like Bitunix)
    if exchange.lower() in NON_CCXT_EXCHANGES:
        # Non-CCXT exchanges are custom implementations, assume they're valid
        return True, "", None
    ...
    try:
        ...
        try:
            ...
            return False, f"Exchange {exchange} not found in ccxt", None
...
def _build_exchange_list_entry(
    ...
    result: ValidExchangesType = {
        ...
        if ex_mod and getattr(ex_mod, "alias", False)
        else None,
