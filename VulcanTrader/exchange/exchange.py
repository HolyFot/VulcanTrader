...
from VulcanTrader.exchange.common import (
...
from VulcanTrader.exchange.exchange_ws import ExchangeWS
from VulcanTrader.util.misc import (
    chunks,
    deep_merge_dicts,
    file_dump_json,
    file_load_json,
...
from VulcanTrader.util import FtTTLCache, PeriodicCache, dt_from_ts, dt_now
from VulcanTrader.util.datetime_helpers import dt_humanize_delta, dt_ts, format_ms_time
...
class Exchange:
    ...
    # This defines defaults, which can be selectively overridden by subclasses using trader_has
    # or by specifying them in the configuration.
    trader_has_default: TraderHas = {
    ...
    def __init__(
        ...
        *,
        ...
        # Deep merge trader_has with default trader_has options
        # Must be called before trader_has is used.
        self.buildtrader_has(exchange_conf)
        ...
        remove_exchange_credentials(
            exchange_conf,
            not self.trader_has["always_require_api_keys"] and config.get("dry_run", False),
        )
        self.log_responses = exchange_conf.get("log_responses", False)

        # Assign this directly for easy access
        self._ohlcv_partial_candle = self.trader_has["ohlcv_partial_candle"]
        ...
        _has_watch_ohlcv = self.exchange_has("watchOHLCV") and self.trader_has["ws_enabled"]
        if (
        ...
        if validate:
            ...
            logger.warning("[exchange-init] reload_markets took %.2fs", _t.perf_counter() - _t0)
            _t0 = _t.perf_counter()
            self.validate_config(config)
            logger.warning("[exchange-init] validate_config took %.2fs", _t.perf_counter() - _t0)

        if self.trading_mode != TradingMode.SPOT and load_leverage_tiers:
            import time as _t
            _t0 = _t.perf_counter()
            self.fill_leverage_tiers()
            logger.warning("[exchange-init] fill_leverage_tiers took %.2fs", _t.perf_counter() - _t0)
        import time as _t
        _t0 = _t.perf_counter()
        self.trader_additional_exchange_init()
        logger.warning("[exchange-init] trader_additional_exchange_init took %.2fs", _t.perf_counter() - _t0)
    ...
    def __del__(self):
        ...
        try:
            ...
            # Ignore errors during garbage collection - handles may already be invalid
            pass
    ...
    def close(self):
        try:
            ...
            # Ignore errors during websocket cleanup - handles may be invalid during shutdown
            pass

        logger.debug("Exchange object destroyed, closing async loop")

        # Close async ccxt session
        try:
            if (
            ...
            # Ignore errors during async session close - connection may already be closed
            logger.debug(f"Exception closing async session (expected during shutdown): {e}")

        # Close websocket async session
        try:
            if (
                self._ws_async
            ...
            # Ignore errors during websocket session close - connection may already be closed
            logger.debug(f"Exception closing ws session (expected during shutdown): {e}")
        ...
        try:
            ...
            # Ignore errors during loop close - may already be closed or invalid
            logger.debug(f"Exception closing event loop (expected during shutdown): {e}")
    ...
    def validate_config(self, config: Config) -> None:
        ...
        if self._config.get("runmode") in TRADE_MODES:
    ...
    def _ccxt_config(self) -> dict:
        ...
        if self.trading_mode == TradingMode.MARGIN:
            ...
            return {"options": {"defaultType": self.trader_has["ccxt_futures_name"]}}
        else:
            return {}
    ...
    def timeframes(self) -> list[str]:
        market_type = (
            ...
            else self.trader_has["ccxt_futures_name"]
        )
    ...
    def trader_additional_exchange_init(self) -> None:
        """
    ...
    def ohlcv_candle_limit(
        ...
        if not isinstance(ccxt_val, float | int):
            ccxt_val = 500
        fallback_val = self.trader_has.get("ohlcv_candle_limit", ccxt_val)
        if candle_type == CandleType.FUNDING_RATE:
            fallback_val = self.trader_has.get("funding_fee_candle_limit", fallback_val)
        return int(
            self.trader_has.get("ohlcv_candle_limit_per_timeframe", {}).get(
    ...
    def market_is_future(self, market: dict[str, Any]) -> bool:
        return (
            market.get(self.trader_has["ccxt_futures_name"], False) is True
    ...
    def market_is_tradable(self, market: dict[str, Any]) -> bool:
        """
        Check if the market symbol is tradable by VulcanTrader.
    ...
    def _order_contracts_to_amount(self, order: CcxtOrder) -> CcxtOrder:
        if "symbol" in order and order["symbol"] is not None:
            ...
            if contract_size != 1:
                for prop in self.trader_has.get("order_props_in_contracts", []):
    ...
    def _markets_cache_path(self):
        from pathlib import Path
        datadir = self._config.get("datadir") or Path("user_data/data")
        cache_dir = Path(datadir) / ".exchange_cache"
        cache_dir.mkdir(parents=True, exist_ok=True)
        # Separate cache per exchange + trading mode (spot vs futures markets differ).
        return cache_dir / f"{self._api.id}_{self.trading_mode.value}_markets.pkl"

    def _try_load_markets_from_cache(self, max_age_hours: int = 24) -> bool:
        """Restore markets/currencies onto both ccxt clients from a pickle cache.
        Returns True on hit so callers can skip the network round-trip."""
        import pickle
        import time as _t
        ...
        try:
            ...
            # Pairs the live exchange genuinely doesn't have — recorded after a
            # fresh fetch so we don't refetch every startup chasing them.
            known_missing = set(payload.get("known_missing") or [])
            # If the configured pair_whitelist references a symbol not in the
            # cache (and not already known to be unavailable), force a fresh
            # fetch instead of running with stale data.
            wl = list(self._config.get("exchange", {}).get("pair_whitelist") or [])
            missing = [p for p in wl if p not in markets and p not in known_missing]
            if missing:
                logger.warning(
                    "[reload_markets] cache missing %d configured pair(s) %s; refetching.",
                    len(missing), missing[:5],
                )
                return False
            for client in (self._api_async, self._api):
                client.set_markets(markets, currencies)
            if self._exchange_ws and self._ws_async is not None:
                self._ws_async.set_markets(markets, currencies)
            logger.warning(
                "[reload_markets] loaded %d markets from cache (age=%.1fh): %s",
                len(markets), age_h, path,
            )
            return True
        except Exception as exc:
            logger.warning("Markets cache unusable (%s); refetching.", exc)
            return False

    def _force_load_markets_from_cache(self) -> bool:
        """Fallback: load markets from cache ignoring age and pair-whitelist checks.
        Used when the live API call fails so the exchange object has at least some
        market data and doesn't crash with 'Could not load markets'."""
        import pickle
        path = self._markets_cache_path()
        if not path.is_file():
            return False
        try:
            ...
            logger.warning(
                "[reload_markets] API failed; using stale cache with %d markets as fallback: %s",
                len(markets), path,
            )
            return True
        except Exception as exc:
            logger.warning("Markets cache unusable as fallback (%s).", exc)
            return False

    def _save_markets_cache(self) -> None:
        import pickle
        try:
            markets = self._api_async.markets or {}
            wl = list(self._config.get("exchange", {}).get("pair_whitelist") or [])
            known_missing = sorted(p for p in wl if p not in markets)
            if known_missing:
                logger.warning(
                    "Configured pair_whitelist contains %d symbol(s) not on the exchange: %s",
                    len(known_missing), known_missing[:5],
                )
            payload = {
                "markets": markets,
                "currencies": getattr(self._api_async, "currencies", {}) or {},
                "known_missing": known_missing,
            }
            with open(self._markets_cache_path(), "wb") as fh:
                pickle.dump(payload, fh)
        except Exception as exc:
            logger.debug("Failed to write markets cache: %s", exc)
    ...
    def reload_markets(self, force: bool = False, *, load_leverage_tiers: bool = True) -> None:
        ...
        try:
            ...
            _t0 = _t.perf_counter()
            cache_hit = is_initial and self._try_load_markets_from_cache()
            if not cache_hit:
                retrier(self._load_async_markets, retries=retries)(reload=True)
                logger.warning("[reload_markets] ccxt load_markets took %.2fs", _t.perf_counter() - _t0)
            self._markets = self._api_async.markets
            if not cache_hit:
            ...
            if not cache_hit:
                self._save_markets_cache()

            if is_initial and self.trader_has["needs_trading_fees"]:
                _t0 = _t.perf_counter()
                self._trading_fees = self.fetch_trading_fees()
                logger.warning("[reload_markets] fetch_trading_fees took %.2fs", _t.perf_counter() - _t0)
            ...
            # Fallback: use stale cache so parallel backtest workers don't crash
            # with 'Could not load markets' when the API is rate-limited.
            if is_initial and not self._markets:
                self._force_load_markets_from_cache()
    ...
    def validate_timeframes(self, timeframe: str | None) -> None:
        ...
        if (
            ...
            raise ConfigurationError("Timeframes < 1m are currently not supported by VulcanTrader.")
    ...
    def validate_stop_ordertypes(self, order_types: dict) -> None:
        ...
        """
        if order_types.get("stoploss_on_exchange") and not self.trader_has.get(
            "stoploss_on_exchange", False
        ):
            raise ConfigurationError(f"On exchange stoploss is not supported for {self.name}.")
        if self.trading_mode == TradingMode.FUTURES:
            price_mapping = self.trader_has.get("stop_price_type_value_mapping", {}).keys()
            if (
    ...
    def validate_pricing(self, pricing: dict) -> None:
        ...
        if not pricing.get("use_order_book", False) and (
            not self.exchange_has("fetchTicker") or not self.trader_has["tickers_have_price"]
        ):
    ...
    def validate_order_time_in_force(self, order_time_in_force: dict) -> None:
        ...
        """
        if any(
            v.upper() not in self.trader_has["order_time_in_force"]
    ...
    def validate_orderflow(self, exchange: dict) -> None:
        if exchange.get("use_public_trades", False) and (
            not self.exchange_has("fetchTrades") or not self.trader_has["trades_has_history"]
        ):
            raise ConfigurationError(
    ...
    def validate_freqai(self, config: Config) -> None:
        ...
        if not override and freqai_enabled and not self.trader_has["ohlcv_has_history"]:
            raise ConfigurationError(
                f"Historic OHLCV data not available for {self.name}. Can't use freqAI."
            )
        elif override and freqai_enabled and not self.trader_has["ohlcv_has_history"]:
            logger.warning(
    ...
    def validate_required_startup_candles(self, startup_candles: int, timeframe: str) -> int:
        ...
        if self.trader_has["ohlcv_has_history"]:
    ...
    def validate_trading_mode_and_margin_mode(
        ...
        """
        Checks if VulcanTrader can perform trades using the configured
        trading mode(Margin, Futures) and MarginMode(Cross, Isolated)
        Throws OperationalException:
            If the trading_mode/margin_mode type are not supported by VulcanTrader on this exchange
        """
        ...
        if allow_none_margin_mode and margin_mode is None:
            ...
            if not any(
                ...
                raise ConfigurationError(
                    f"VulcanTrader does not support '{trading_mode}' on {self.name}."
                )
        ...
        if not allow_none_margin_mode and (
            ...
            raise ConfigurationError(
                f"VulcanTrader does not support '{mm_value}' '{trading_mode}' on {self.name}."
            )

    @classmethod
    def combinetrader_has(cls, include_futures: bool) -> TraderHas:
        """
        Combine all trader_has options from the class hierarchy.
        Child classes override parent classes.
        Doesn't apply overrides from the configuration.
        """
        trader_has = deep_merge_dicts(cls.trader_has, deepcopy(cls.trader_has_default))

        if include_futures:
            trader_has = deep_merge_dicts(cls.trader_has_futures, trader_has)
        return trader_has

    def buildtrader_has(self, exchange_conf: ExchangeConfig) -> None:
        """
        Deep merge trader_has with default trader_has options
        and with exchange_conf.trader_has_params if available.
        This is called on initialization of the exchange object.
        It must be called before trader_has is used.
        """
        self.trader_has = self.combinetrader_has(include_futures=self.trading_mode == TradingMode.FUTURES)

        if exchange_conf.get("trader_has_params"):
            self.trader_has = deep_merge_dicts(exchange_conf.get("trader_has_params"), self.trader_has)
            logger.info("Overriding exchange.trader_has with config params, result: %s", self.trader_has)

    def get_option(self, param: str, default: Any | None = None) -> Any:
        """
        Get parameter value from trader_has
        """
        return self.trader_has.get(param, default)
    ...
    def exchange_has(self, endpoint: str) -> bool:
        ...
        :return: bool
        """
        if endpoint in self.trader_has.get("exchange_has_overrides", {}):
            return self.trader_has["exchange_has_overrides"][endpoint]
    ...
    def create_dry_run_order(
        ...
        if stop_loss:
            ...
            dry_order[self.trader_has["stop_price_prop"]] = stop_price or dry_order["price"]
            # Workaround to avoid filling stoploss orders immediately
            dry_order["trader_order_type"] = "stoploss"
        ...
        if dry_order["type"] == "market" and not dry_order.get("trader_order_type"):
    ...
    def check_dry_limit_order_filled(
        ...
        """
        if order["status"] != "closed" and order.get("trader_order_type") == "stoploss":
            pair = order["symbol"]
            if not orderbook and self.exchange_has("fetchL2OrderBook"):
                orderbook = self.fetch_l2_order_book(pair, 20)
            price = safe_value_fallback(order, self.trader_has["stop_price_prop"], "price")
        ...
        if (
            ...
            and not order.get("trader_order_type")
        ):
            pair = order["symbol"]
            if self._dry_is_price_crossed(pair, order["side"], order["price"], orderbook):
                ob = orderbook or (self.fetch_l2_order_book(pair, 1) if self.exchange_has("fetchL2OrderBook") else None)
                market_price = (
                    ob["asks"][0][0] if order["side"] == "buy" and ob and ob.get("asks")
                    else ob["bids"][0][0] if ob and ob.get("bids")
                    else None
                )
                logger.debug(
                    f"[DRY-RUN] Limit order {order.get('id')} FILLED: "
                    f"limit_price={order['price']}, market_price={market_price}, "
                    f"side={order['side']}, amount={order['amount']}, pair={pair}"
                )
                order.update(
                    {
                        "status": "closed",
    ...
    def fetch_dry_run_order(self, order_id: str) -> CcxtOrder:
        ...
        try:
            ...
            from VulcanTrader.persistence import Order

            order_obj = Order.order_by_id(order_id)
            if order_obj:
                order = order_obj.to_ccxt_object(self.trader_has["stop_price_prop"])
                order = self.check_dry_limit_order_filled(order)
    ...
    def _order_needs_price(self, side: BuySell, ordertype: str) -> bool:
        return (
            ...
            or self.trader_has.get("marketOrderRequiresPrice", False)
        )
    ...
    def stoploss_adjust(self, stop_loss: float, order: CcxtOrder, side: str) -> bool:
        ...
        """
        if not self.trader_has.get("stoploss_on_exchange"):
            raise OperationalException(f"stoploss is not implemented for {self.name}.")
        price_param = self.trader_has["stop_price_prop"]
    ...
    def _get_stop_order_type(self, user_order_type) -> tuple[str, str]:
        available_order_Types: dict[str, str] = self.trader_has["stoploss_order_types"]
    ...
    def _get_stop_params(self, side: BuySell, ordertype: str, stop_price: float) -> dict:
        ...
        params.update({self.trader_has["stop_price_param"]: stop_price})
        return params
    ...
    def create_stoploss(
        ...
        requires `trader_has['stoploss_order_types']` to be set as a dict mapping limit and market
        ...
        """
        if not self.trader_has["stoploss_on_exchange"]:
            raise OperationalException(f"stoploss is not implemented for {self.name}.")
        ...
        try:
            ...
            if self.trading_mode == TradingMode.FUTURES:
                ...
                if "stoploss_price_type" in order_types and "stop_price_type_field" in self.trader_has:
                    price_type = self.trader_has["stop_price_type_value_mapping"][
                        order_types.get("stoploss_price_type", PriceType.LAST)
                    ]
                    params[self.trader_has["stop_price_type_field"]] = price_type
    ...
    def fetch_orders(
        ...
        if self._config["dry_run"]:
            return []
        if (limit := self.trader_has.get("fetch_orders_limit_minutes")) is not None:
            orders = []
            while since < dt_now():
    ...
    def get_conversion_rate(self, coin: str, currency: str) -> float | None:
        ...
        if (proxy_coin := self.trader_has["proxy_coin_mapping"].get(coin, None)) is not None:
            coin = proxy_coin
        if (proxy_currency := self.trader_has["proxy_coin_mapping"].get(currency, None)) is not None:
    ...
    def fetch_l2_order_book(self, pair: str, limit: int = 100) -> OrderBook:
        ...
        limit1 = self.get_next_limit_in_list(
            limit,
            self.trader_has["l2_limit_range"],
            self.trader_has["l2_limit_range_required"],
            self.trader_has["l2_limit_upper"],
        )
        try:
    ...
    def get_fee(
        ...
        try:
            ...
            raise OperationalException(
                f"Could not get fee info for {symbol}: market not found. Message: {e}"
            ) from e
    ...
    def _build_coroutine(
        ...
        if not since_ms and (self.trader_has["ohlcv_require_since"] or not_all_data):
    ...
    async def _async_get_candle_history(
        ...
        try:
            ...
            params = deepcopy(self.trader_has.get("ohlcv_params", {}))
            candle_limit = self.ohlcv_candle_limit(
    ...
    def refresh_latest_trades(
        ...
        *,
        ...
        :return: Dict of [{(pair, timeframe): Dataframe}]
        """
        from VulcanTrader.data.history import get_datahandler
    ...
    async def _async_fetch_trades(
        ...
        """
        try:
            trades_limit = self.trader_has["trades_limit"]
    ...
    def _get_trade_pagination_next_value(self, trades: list[dict]):
        ...
        if not trades:
            return None
        if self.trader_has["trades_pagination"] == "id":
    ...
    async def _async_get_trade_history_id(
        ...
        """
        Asynchronously gets trade history using fetch_trades
        use this when exchange uses id-based iteration (check `self.trader_has["trades_pagination"]`)
        :param pair: Pair to fetch trade data for
        :param since: Since as integer timestamp in milliseconds
        ...
        has_overlap = self.trader_has.get("trades_pagination_overlap", True)
        ...
        while True:
            try:
                t, from_id_next = await self._async_fetch_trades(
                    pair, params={self.trader_has["trades_pagination_arg"]: from_id}
                )
                if t:
                    trades.extend(t[x])
    ...
    async def _async_get_trade_history_time(
        ...
        """
        Asynchronously gets trade history using fetch_trades,
        when the exchange uses time-based iteration (check `self.trader_has["trades_pagination"]`)
        :param pair: Pair to fetch trade data for
        :param since: Since as integer timestamp in milliseconds
        :param until: Until as integer timestamp in milliseconds
        returns tuple: (pair, trades-list)
    ...
    async def _async_get_trade_history(
        ...
        if self.trader_has["trades_pagination"] == "time":
            return await self._async_get_trade_history_time(pair=pair, since=since, until=until)
        elif self.trader_has["trades_pagination"] == "id":
    ...
    def _set_leverage(
        ...
        if self._config["dry_run"] or not self.exchange_has("setLeverage"):
            ...
            return
        if self.trader_has.get("floor_leverage", False) is True:
    ...
    def _fetch_and_calculate_funding_fees(
        ...
        timeframe = self.trader_has["mark_ohlcv_timeframe"]
        timeframe_ff = self.trader_has["funding_fee_timeframe"]
        mark_price_type = CandleType.from_string(self.trader_has["mark_ohlcv_price"])
    ...
    def calculate_funding_fees(
        ...
        if not df.empty:
            ...
            # where open/close dates can have microsecond precision - but the column may not.
    ...
    def dry_run_liquidation_price(
        ...
        if self.trading_mode == TradingMode.FUTURES and self.margin_mode == MarginMode.ISOLATED:
            if market["inverse"]:
                raise OperationalException("VulcanTrader does not yet support inverse contracts")
